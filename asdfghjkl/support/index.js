...
function sendTestAfterMetrics(test) {
  ...
  cy.task(`currents:test:after:run`, (0, import_fast_safe_stringify.default)(pickTestData(test)), {
    log: false
  });
}
function sendTestBeforeMetrics(test) {
  beforeReportedTests.push(getTestHash(test));
  cy.task(`currents:test:before:run`, (0, import_fast_safe_stringify.default)(pickTestData(test)), {
    log: false
  });
}
